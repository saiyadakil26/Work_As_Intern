...
const blog_model= async (ctx,next)=>{
    ...
    data["create_by"]=creater_data.id
    data["owned_by"]=creater_data.owner
    data["_id"]= uuid.v4()
...
const blog_update_model= async (ctx,next)=>{
    ...
    try {
        ...
        let res=await find_blog({_id:data.id},{},{})
        if(res.length !=1){
            response_send(ctx,403,{"error":"Blog Is not available","is_Posted":false})
            return
        }
        else if (res[0].owned_by!=creater_data.owner){
...
const delete_blog_model=async (ctx,next)=>{
    ...
    try {
        ...
        let res=await find_blog({_id:ctx.request.query.id},{},{})
        if(res.length !=1){
            response_send(ctx,403,{"error":"Blog Is not available","is_Posted":false})
            return
        }
...
const like_model=async(ctx,next)=>{
    let data=ctx.request.query.id
    let res=await find_blog({_id:data},{},{})
        if(res.length !=1){
            response_send(ctx,403,{"error":"Blog Is not available","is_Posted":false})
            return
        }
